...
已有研究分别处理了几类相邻问题：有人整理实验上测得的 inactive 化合物，有人研究分子数据集和 benchmark 的偏差，有人利用失败实验训练模型，也有人研究从化学文献中抽取结构化信息。最近还出现了专门面向生物惰性小分子的资源。
...
这说明“数据库里完全没有阴性数据”是不准确的。更准确的问题是：这些数据是否足够完整、是否代表真实实验空间、是否保留了决定标签含义的 assay 和实验条件。
...
目前的不足不在于“能不能从论文中抽一句话”，而在于能否正确完成以下关联：
...
> 阴性数据、失败实验、分子数据偏差和化学文献抽取都已经有先行研究，甚至已经出现了专门的 inactive compound 资源；但是，面向分子科学的、以观察为单位、区分 unknown 与 negative、保留实验上下文和证据出处，并用真实任务验证数据价值的完整方案，仍然没有被充分解决。
